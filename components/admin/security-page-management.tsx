...
import { Save, Upload, X, Plus, Trash2, Edit, Eye, EyeOff } from "lucide-react";
...
export function SecurityPageManagement() {
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [content, setContent] = useState<PageContent | null>(null);
  const [activeTab, setActiveTab] = useState<"hero" | "intro" | "categories" | "honey" | "gallery" | "resolution" | "faq" | "cta">("hero");
  ...
  return (
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-dark-blue mb-2">Security & Surveillance Page Management</h1>
          <p className="text-light-gray">Manage all content for the Security & Surveillance service page</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 border-b border-lavender-light overflow-x-auto pb-2">
        {[
          ...
          { id: "resolution", label: "Resolution Comparison" },
          { id: "faq", label: "FAQs" },
      ...
      {/* Resolution Comparison Tab */}
      {activeTab === "resolution" && (
        <div className="bg-white rounded-lg border border-lavender-light p-6">
          <h2 className="text-xl font-bold text-dark-blue mb-4">Resolution Comparison</h2>
          <div className="grid grid-cols-2 gap-4">
            {content.resolutionComparison.map((res, idx) => (
              <div key={idx} className="border border-lavender-light rounded-lg p-4">
                <Input
                  value={res.label}
                  onChange={(e) => {
                    const resolutions = [...content.resolutionComparison];
                    resolutions[idx].label = e.target.value;
                    setContent({ ...content, resolutionComparison: resolutions });
                  }}
                  className="mb-2"
                />
                <div className="flex gap-2">
                  <Input
                    ...
                    onChange={(e) => {
                      const resolutions = [...content.resolutionComparison];
                      resolutions[idx].image = e.target.value;
                      setContent({ ...content, resolutionComparison: resolutions });
                    }}
                    placeholder="Image URL"
                  />
                ...
                <div className="mt-2">
                  <Button
                    ...
                    onClick={() => {
                      const resolutions = [...content.resolutionComparison];
                      resolutions[idx].visible = !resolutions[idx].visible;
                      setContent({ ...content, resolutionComparison: resolutions });
                    }}
                  >
                    {res.visible ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                  </Button>
                </div>
              </div>
            ))}
